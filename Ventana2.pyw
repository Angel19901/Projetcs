...
def Ventana2():
    ...
    ventana2 = tkinter.Tk()
    ventana2.title("Expediente") # Título de la ventana
    ancho_valor = ventana2.winfo_screenwidth()# Creo una variable que determine el ancho del monitor
    altura_valor = ventana2.winfo_screenheight()# Creo una variable que determine el alto del monitor
    ventana2.geometry("%dx%d+0+0"%(ancho_valor, altura_valor))# Paso como valor en ancho y alto al métod "geometry"
    ventana2.config(bg="#0c3a56")

    # Título que aparecerá en la ventana
    etiqueta = tkinter.Label(ventana2, text="Expediente")
    etiqueta.pack()
    etiqueta.config(font=("Arial", 30, "bold"), fg="white",  bg="#0c3a56")

    #Creación del marco que contendrá al scroll
    a1 = LabelFrame(ventana2)
